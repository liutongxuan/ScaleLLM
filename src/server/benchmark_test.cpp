...
class LLM {
 public:
  ...
  void generate(const std::vector<std::string>& batched_prompt) {
    ...
    // create a request
    llm::Request request("request_id_0", std::vector<int>());
    request.echo = true;
    ...
    for (int64_t i = 0; i < FLAGS_max_seq_len; ++i) {
      ...
      for (int64_t i = 0; i < num_seqs; ++i) {
        ...
        //std::cout << sequence->decode_delta_text(sequence->num_tokens(), *tokenizer_)
        //          << std::flush;
      }
    }
    std::cout << "request cost:" << request_cost / kMicrosToSeconds << std::endl;
  }
