...
VLMHandler::~VLMHandler() { reset(); }

std::future<bool> VLMHandler::schedule_async(torch::Tensor image,
                                             std::string prompt,
                                             SamplingParams sp,
                                             Priority priority,
                                             bool stream,
...
std::future<bool> VLMHandler::schedule(torch::Tensor image,
                                       std::string prompt,
                                       SamplingParams sp,
                                       Priority priority,
                                       bool stream,
  ...
  queue_.push([this,
    ...
    auto request =
        create_request(tid, std::move(image), std::move(prompt), sp, priority, stream, callback);
    if (!request) {
      promise.set_value(false);
      return;
    }
...
void VLMHandler::handling_loop(size_t tid) {
  while (true) {
...
void VLMHandler::run_until_complete() {
  const bool running = running_.load(std::memory_order_relaxed);
  CHECK(!running) << "Handler is already running";
...
std::unique_ptr<Request> VLMHandler::create_request(size_t tid,
                                                    torch::Tensor image,
                                                    std::string prompt,
...
std::vector<int32_t> VLMHandler::encode(const std::string& text) {
  std::vector<int> tokens;
  engine_->tokenizer()->encode(text, &tokens);
  return tokens;
}

std::string VLMHandler::decode(const std::vector<int32_t>& tokens,
                               bool skip_special_tokens) {
  return engine_->tokenizer()->decode(tokens, skip_special_tokens);
}

void VLMHandler::reset() {
  stop();
