...
bool VLMWorker::init_model(torch::ScalarType dtype,
...
bool VLMWorker::init_kv_cache(const std::vector<int64_t>& kv_cache_shape) {
...
bool VLMWorker::capture_cuda_graphs() {
  CHECK(model_ != nullptr) << "Model is not initialized.";
...
void VLMWorker::load_state_dict(const StateDict& state_dict) {
  CHECK(model_ != nullptr) << "Model is not initialized.";
  model_->load_state_dict(state_dict);
}

void VLMWorker::verify_loaded_weights() const {
  CHECK(model_ != nullptr) << "Model is not initialized.";
  model_->verify_loaded_weights();
}

std::tuple<int64_t, int64_t> VLMWorker::profile_device_memory() {
  CHECK(model_ != nullptr) << "Model is not initialized.";
...
ModelOutput VLMWorker::execute_model(const ModelInput& inputs,
                                     const Tensor& input_embedding) {
  ...
  auto hidden_states = model_runner_->forward(
      flatten_tokens, input_embedding, flatten_positions, kv_caches_, params);
...
VLMWorker::profile_device_memory_async() {
...
folly::SemiFuture<ModelOutput> VLMWorker::execute_model_async(
...
folly::SemiFuture<bool> VLMWorker::init_model_async(
...
folly::SemiFuture<bool> VLMWorker::init_kv_cache_async(
...
folly::SemiFuture<bool> VLMWorker::capture_cuda_graphs_async() {
  folly::Promise<bool> promise;
  auto future = promise.getSemiFuture();
  threadpool_.schedule([this, promise = std::move(promise)]() mutable {
...
folly::SemiFuture<folly::Unit> VLMWorker::load_state_dict_async(
