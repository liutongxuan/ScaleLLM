...
VLMEngine::VLMEngine(const Options& options) : options_(options) {
  ...
  worker_ = std::make_unique<Worker>(parallel_args, devices[i], runner_options);
}
...
bool VLMEngine::capture_cuda_graphs() {
  ...
  return worker_->capture_cuda_graphs();
}
...
ModelOutput VLMEngine::execute_model(Batch& batch) {
  ...
  // only one worker, call blocking forward
  auto model_output =
      worker_->execute_model(model_inputs, batch.get_input_embedding());
