...
class AttentionValidateTest
    : public ::testing::TestWithParam<std::tuple<torch::Device,
                                                 ...
                                                 bool /*alibi*/,
                                                 int32_t /*num_splits*/>> {};
TEST_P(AttentionValidateTest, OneSequenceValidateTestBasic) {
  const auto& [device,
               dtype,
               batch_size,
               block_size,
...
INSTANTIATE_TEST_SUITE_P(
    OneSequenceValidateTestBasic,
    AttentionValidateTest,
    ::testing::Combine(
        ::testing::Values(torch::kCUDA),
        ::testing::Values(torch::kHalf, torch::kBFloat16),
        ::testing::Values(1),                                // batch_size
        ::testing::Values(256),                              // block_size
        ::testing::Values(10),                               // q_max_seq_len
        ::testing::Values(100),                              // kv_max_seq_len
        ::testing::Values(6),                                // n_heads
        ::testing::Values(6),                                // n_kv_heads
        ::testing::Values(32),                               // head_dim
        ::testing::Values(0.9),                              // scale
        ::testing::Values(false),                            // alibi
        ::testing::Values(1)                                 // num_splits
        ));

TEST(AttentionSpeculativeTest, LLMPrefillTest) {
  torch::Device device(torch::kCUDA);
  torch::ScalarType dtype(torch::kHalf, torch::kBFloat16);
  int64_t block_size = 256;
  int64_t q_max_seq_len = 10;
  int64_t kv_max_seq_len = 100;
  int64_t n_heads = 6;
  int64_t n_kv_heads = 6;
  int64_t head_dim = 32;
  float scale = 0.9;
  const int32_t n_blocks =
    (kv_max_seq_len + block_size - 1) / block_size * 2;
  std::vector<int32_t> available_block_ids(n_blocks);
  for (int32_t i = 0; i < n_blocks; ++i) {
    available_block_ids[i] = i;
  }
